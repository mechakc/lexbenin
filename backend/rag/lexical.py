...
PREFIX_LEN = 5  # longueur de préfixe pour la tolérance aux fautes/anglicismes
...
class LexicalIndex:
    """Index BM25 en mémoire pour une collection (un pays/domaine), avec
    tolérance aux fautes de frappe et anglicismes courants (ex: "access" pour
    "accès") via une correspondance de préfixe sur le vocabulaire du corpus."""
    ...
    def __init__(self, ids: list[str], texts: list[str]):
        ...
        # Vocabulaire du corpus + index par préfixe de 5 caractères, pour
        # retomber sur un mot réel du corpus quand un mot de la question ne
        # matche rien exactement (faute de frappe, anglicisme -- ex: "access"
        # tapé au lieu de "accès" -> "acces" une fois les accents retirés).
        self.vocabulary: set[str] = {tok for doc in tokenized_corpus for tok in doc}
        self._prefix_index: dict[str, list[str]] = defaultdict(list)
        for word in self.vocabulary:
            if len(word) >= PREFIX_LEN:
                self._prefix_index[word[:PREFIX_LEN]].append(word)

    def _normalize_query_tokens(self, tokens: list[str]) -> list[str]:
        """Remplace tout token absent du vocabulaire par un mot réel du corpus
        partageant le même préfixe de 5 caractères, s'il en existe un. Laisse
        le token tel quel si aucun candidat n'est trouvé (ou s'il est trop
        court pour qu'un préfixe de 5 caractères ait un sens)."""
        normalized = []
        for tok in tokens:
            if tok in self.vocabulary or len(tok) < PREFIX_LEN:
                normalized.append(tok)
                continue
            candidates = self._prefix_index.get(tok[:PREFIX_LEN])
            normalized.append(candidates[0] if candidates else tok)
        return normalized
    ...
    def search(self, query: str, top_k: int = 10) -> list[tuple[str, float]]:
        ...
        tokenized_query = self._normalize_query_tokens(tokenize(query))
